...
type CurrState = (Int,NNonEmpty Table4,[VarNum]) -- block num, defined variables(inner scope first), used variable num
...
convert4'((n ,st    ,ls),(Bot(WHI,ide,Nodes v):xs)) = case (lookup' ide (toList st)) of
   Just k                                       -> 
    (\x->"mov "++show k++"; loop; "++x)<$>convert4' ((n+1,M.empty `cons` st,ls),v)<++>Right("mov "++show k++"; pool; ")<++>convert4'((n,st,ls),xs)
   Nothing                                      -> Left $newErrorMessage (Message$"identifier "++show ide++"is not defined")(newPos "step4" 0 0)