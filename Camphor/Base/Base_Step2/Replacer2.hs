...
{-  -------------------------------------------------------------------------------
   ...
   ***************************
----------------------------------------------------------------------------------}  
replacer3 :: UserState -> NonEmpty MacroId -> SourcePos -> SimpleSent -> ReplTable -> CollisionTable -> Either ParseError ([SimpleSent],CollisionTable)
-- simple ones --
replacer3 _ _ _ Scolon   _     = wrp[Scolon]
replacer3 _ _ _ (Sp x)   _     = wrp[Sp x]
replacer3 _ _ _ (Comm x) _     = wrp[Comm x]
replacer3 _ _ _ (Pragma x) _   = wrp[Pragma x]
...
-- calls --  
replacer3 stat (n:|ns) pos (Call1 ident valuelist) table 
...
replacer3 stat (n:|ns) pos (Call1WithBlock ident valuelist pos2 block) table = \coltable -> do  -- ident is not replaced
 ...
 where
  ...
  makeNewBlock2 (Single _ ssent) = \clt -> do
   (res,nclt) <- replacer3 stat (n:|ns) pos ssent table clt
...
replacer3 stat (n:|ns) pos (Call2 oper valuelist1 valuelist2) table = do
...
replacer3 stat narr pos (Call3 op valuelist1 valuelist2) table = \coltable -> do
 isValidCall3 pos op valuelist2 stat
 replacer3 stat narr pos (Call2 op valuelist1 valuelist2) table coltable

replacer3 stat narr pos (Call4 []     valuelist ) table = replacer3 stat narr pos (Call5 valuelist) table
replacer3 stat narr pos (Call4 (x:xs) valuelist2) table = \coltable -> do
 (valuelist1,op) <- getCall4Left pos (x:|xs) stat
 replacer3 stat narr pos (Call2 op valuelist1 valuelist2) table coltable

replacer3 _ _ _ (Call5 (SepList(Constant _,[]))) _ = wrp[Scolon]
replacer3 _ _ _ (Call5 (SepList(Var ident,[]))) table = case M.lookup ident table of
 Nothing -> \clt -> return([Call5(SepList(newIdentIP clt ident,[]))],clt)
 Just x  -> wrp[Call5(SepList(x                   ,[]))]
replacer3 stat narr pos (Call5 (SepList(x,ov:ovs))) table = \coltable -> do
 (oper,vlist1,vlist2) <- getCall5Result pos (x,ov:|ovs) stat
 replacer3 stat narr pos (Call2 oper vlist1 vlist2) table coltable
 
--- built-in (+=) & (-=) ---
replacer3 _ _ pos (Pleq (Var v1) (Constant v2)) table = case M.lookup v1 table of
 Nothing        -> \clt -> return([Pleq (newIdentIP clt v1)(Constant v2)],clt)
 Just v@(Var _) -> wrp[Pleq v (Constant v2)]
 Just _         -> err$cantbeleft v1 "+=" pos 

replacer3 _ _ pos (Mneq (Var v1) (Constant v2)) table = case M.lookup v1 table of
...
replacer3 stat ns pos (Pleq (Var v1) (Var v2)) table = basis Pleq "+=" (stat,ns,pos,v1,v2,table)
replacer3 stat ns pos (Mneq (Var v1) (Var v2)) table = basis Mneq "-=" (stat,ns,pos,v1,v2,table)
...
replacer3 stat ns pos (Rd (Var ident)) table = case M.lookup ident table of
 Nothing        -> \clt -> return([Rd (newIdentIP clt ident)],clt)
 Just v@(Var _) -> wrp[Rd v] 
 Just c         -> replacer3 stat ns pos (Call1 "read" (SepList (c,[]))) table

replacer3 stat ns pos (Wrt (Var ident)) table = case M.lookup ident table of
 Nothing        -> \clt -> return([Wrt (newIdentIP clt ident)],clt)
 Just v@(Var _) -> wrp[Wrt v] 
 Just c         -> replacer3 stat ns pos (Call1 "write" (SepList (c,[]))) table
...
basis :: (Value -> Value -> SimpleSent) -> String
 -> (UserState, NonEmpty MacroId, SourcePos, Ident, Ident, M.Map Ident Value)
...
basis constr op (stat,ns,pos,v1,v2,table) = case M.lookup v1 table of
 ...
 Just v@(Var _) -> res v 
 Just _         -> err$newErrorMessage(Message$show v1++" is a constant and thus cannot be the left side of operator "++show op)pos 
 where
 res :: Value -> CollisionTable -> Either ParseError ([SimpleSent],CollisionTable)
 res v = case M.lookup v2 table of
  Nothing      -> \clt -> replacer3 stat ns pos (Call2 (wrap op) (SepList(v,[])) (SepList(newIdentIP clt v2,[]))) table clt
  Just (Var y) -> \clt -> replacer3 stat ns pos (Call2 (wrap op) (SepList(v,[])) (SepList(Var y            ,[]))) table clt
...
rpl3 :: NonEmpty MacroId -> SourcePos -> Ident -> ValueList -> ReplTable -> UserState -> CollisionTable -> Either ParseError ([SimpleSent],CollisionTable) 
rpl3 ms pos ident valuelist table stat coltable = do
 ...
 case sent' of 
  ...
  Just sent -> do -- obtain what's in the function
   ...
   sents <- rpl1_1 pos sent table2 (mname `cons` ms) stat coltable
   return(sents,coltable)
 
--- First, replace valuelist by the collision table. Then, replace valuelist by the replacement table. Then, expand the macro.
rpl4 ::
 NonEmpty MacroId -> SourcePos -> Oper -> (ValueList,ValueList) -> ReplTable -> UserState -> CollisionTable -> Either ParseError ([SimpleSent],CollisionTable) 
rpl4 ms pos oper (vlist1,vlist2) table stat coltable = do
...
rpl1_1 :: SourcePos -> Sent -> ReplTable -> NonEmpty MacroId -> UserState -> CollisionTable -> Either ParseError [SimpleSent]
rpl1_1 pos sent table2 newMs stat clt = 
 fst <$> rpl1_2 pos sent table2 newMs stat clt  -- we outsiders don't need to know what's in the insiders' collision table.

 
--- simply replace the parameters using the arguments  
rpl1_2 :: SourcePos -> Sent -> ReplTable -> NonEmpty MacroId -> UserState -> CollisionTable -> Either ParseError ([SimpleSent],CollisionTable)
rpl1_2 pos (Single _ ssent) table2 newMs stat  = replacer3 stat newMs pos ssent table2 
rpl1_2 pos (Block  _ xs) table2 newMs stat = \coltable -> do
 (results,clTable) <- forStatM xs (\ssent clt -> rpl1_2 pos ssent table2 newMs stat clt) coltable
...
newIdentIP clt ident = case M.lookup ident clt of
 Nothing -> Var ident
 Just x -> Var x 
...
cantbeleft c str = newErrorMessage(Message$show c++" is a constant and thus cannot be the left side op operator "++show str) 